...
import hpo.choosers.gp_util as gp_util
...
class GPChooser:
    def __init__(self, expt_dir, covar="Matern52", mcmc_iters=10,
                 ...
                 response_shaping='False', shaping_func="hybrid_log", 
                 trade_off=0, v=1.0, alpha=0.3):
    ...
    def get_observations(self, samples, sample_size):
        ...
        size = len(errs)
        if size > sample_size:
            debug("Subsampling {} observations due to GP cubic complexity".format(sample_size))
            nc = []
            ne = []
    ...
    def next(self, samples, af):
        ...
        if self.D == -1:
            num_dims = samples.get_params_dim() # # of parameters dimension
