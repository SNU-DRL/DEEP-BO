...
class HyperOptChooser(object):
    ...
    def next(self, samples, acq_func, use_interim=True):
        ...
        idx = samples.expand(self.selected)
    ...
    def fake_evaluate(self, params):
        ...
        params = self.space_cfg.config.convert('hopt_dict', 'dict', params)
        debug("Selected by HyperOpt: {}".format(params))
        est_loss = 11.0 # XXX:meaningless loss value
        self.selected = self.space_cfg.config.convert('dict', 'arr', params)
...
class HyperoptTrialMaker(object):
    ...
    def create_history(self, completed):
        history = []
        hparams = self.space_cfg.config.get_param_names()
        config = self.space_cfg.config
        
        if len(completed) > 0:
            #debug("completed: {}".format(completed))
            for ci in completed:
                hpv = self.hpvs[ci]
                if ci != None and len(hparams) == len(hpv):
                    h = {}
                    for i in range(len(hparams)):
                        str_index = 0
                        param = hparams[i]
                        val = hpv[i]
                        htype = config.get_value_type(param) 
                        t = config.get_type(param)
                        ...
                        if htype == "categorical" or htype == "preordered":
                            ...
                            cat_vals = config.get_range(param)
                            if t == 'bool':
